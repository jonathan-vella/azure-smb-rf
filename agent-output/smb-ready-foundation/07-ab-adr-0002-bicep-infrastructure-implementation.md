...
**Implemented** - Bicep templates deployed to `infra/bicep/smb-ready-foundation/`.
...
├── firewall.bicep (if scenario == 'firewall' || 'full')
└── vpn-gateway.bicep (if scenario == 'vpn' || 'full')
...
| ---------------- | -------------------------------- | --------------------- | -------------------------------- |
| Resource Groups  | `rg-{workload}-{env}-{region}`   | `rg-hub-smb-swc`      | Shared services use `smb` as env |
...
| NSGs             | `nsg-{workload}-{env}-{region}`  | `nsg-hub-smb-swc`     | Shared services use `smb`        |
...
| Identity   | 2     | 2 Audit            | `smb-identity-*`          |
| Tagging    | 2     | 2 Deny             | `smb-tagging-*`           |
| Governance | 1     | 1 Deny             | `smb-governance-*`        |
| Operations | 2     | 2 AuditIfNotExists | `smb-backup/monitoring-*` |
...
- **POS-005**: `Remove-SmbReadyFoundationPolicies.ps1` enables clean rollback for redeployment
...
- **IMP-004**: If policies already exist, run cleanup script first: `./scripts/Remove-SmbReadyFoundationPolicies.ps1`
...
- **REF-006**: [Azure Naming Conventions](https://learn.microsoft.com/azure/cloud-adoption-framework/ready/azure-best-practices/resource-naming)
