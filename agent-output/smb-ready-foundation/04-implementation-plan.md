# Step 4: Implementation Plan - SMB Ready Foundations

> Generated by bicep-plan agent | 2026-01-27 | Artifact v0.2 (AVM Migration Complete)

## Overview

This implementation plan defines the Bicep Infrastructure as Code for a cost-optimized,
repeatable Azure SMB Ready Foundations designed for on-premises workload migrations targeting SMB customers.
...
- Intermediate management group `smb-rf` (SMB Ready Foundations) under tenant root
...
│   ├── networking-peering-spoke.bicep # VNet peering spoke→hub [Raw - no AVM exists]
│   ├── route-tables.bicep         # UDR for firewall routing [✅ AVM v0.2: route-table 0.5.0]
│   ├── monitoring.bicep           # Log Analytics Workspace [✅ AVM v0.2: operational-insights/workspace 0.15.0]
│   ├── backup.bicep               # Recovery Services Vault [✅ AVM v0.2: recovery-services/vault 0.11.1]
...
> **AVM Migration Complete** (v0.2.0) - All applicable modules now use Azure Verified Modules.

| Module                     | Status             | AVM Modules Used                                        | Notes     |
| -------------------------- | ------------------ | ------------------------------------------------------- | --------- |
| `networking-hub.bicep`     | ✅ AVM v0.3        | VNet 0.7.2, NSG 0.5.2, Private DNS 0.8.0                | Complete  |
| `networking-spoke.bicep`   | ✅ AVM v0.3        | VNet 0.7.2, NSG 0.5.2, NAT Gateway 2.0.1                | Complete  |
| `vpn-gateway.bicep`        | ✅ AVM v0.3        | virtual-network-gateway 0.10.1                          | Complete  |
| `firewall.bicep`           | ✅ Full AVM v0.5   | azure-firewall 0.9.2, firewall-policy 0.3.4, PIP 0.12.0 | Complete  |
| `monitoring.bicep`         | ✅ AVM v0.2        | operational-insights/workspace 0.15.0                   | Complete  |
| `backup.bicep`             | ✅ AVM v0.2        | recovery-services/vault 0.11.1                          | Complete  |
| `route-tables.bicep`       | ✅ AVM v0.2        | route-table 0.5.0                                       | Complete  |
| `networking-peering.bicep` | ❌ Raw (Exception) | **No AVM exists**                                       | Justified |
| `migrate.bicep`            | ❌ Raw (Exception) | **No AVM exists**                                       | Justified |
...
| 4   | Audit ARM VMs            | 1d84d5fb-01f6-4d12-ba4f-4a26081d403d | Audit             | smb-compute-04    |
| 5   | NSG on subnets           | e71308d3-144b-4262-b144-efdc3cc90517 | AuditIfNotExists  | smb-network-01    |
| 6   | Close management ports   | 22730e10-96f6-4aac-ad84-9383d35b5917 | AuditIfNotExists  | smb-network-02    |
| 7   | Restrict NSG ports       | 9daedab3-fb2d-461e-b861-71790eead4f6 | AuditIfNotExists  | smb-network-03    |
| 8   | Disable IP forwarding    | 88c0b9da-ce96-4b03-9635-f29a937e2900 | Deny              | smb-network-04    |
| 9   | Storage HTTPS only       | 404c3081-a854-4457-ae30-26a93ef643f9 | Deny              | smb-storage-01    |
| 10  | No public blob access    | 4fa4b6c0-31ca-4c0d-b10d-24b96f62a751 | Deny              | smb-storage-02    |
| 11  | Storage TLS 1.2          | fe83a0eb-a853-422d-aac2-1bffd182c5d0 | Deny              | smb-storage-03    |
| 12  | Restrict storage network | 34c877ad-507e-4c82-993e-3452a6e0ad3c | Audit             | smb-storage-04    |
| 13  | Storage ARM migration    | 37e0d2fe-28a5-43d6-a273-67d37d1f5606 | Audit             | smb-storage-05    |
| 14  | SQL Azure AD-only        | abda6d70-9778-44e7-84a8-06f9e9f5b64b | Audit             | smb-identity-01   |
| 15  | SQL no public access     | 1b8ca024-1d5c-4dec-8995-b1a932b41780 | Audit             | smb-identity-02   |
| 16  | Require Environment tag  | 871b6d14-10aa-478d-b590-94f262ecfa99 | Deny              | smb-tagging-01    |
| 17  | Require Owner tag        | 871b6d14-10aa-478d-b590-94f262ecfa99 | Deny              | smb-tagging-02    |
| 18  | Allowed locations        | e56962a6-4747-49cd-b67b-bf8b01975c4c | Deny              | smb-governance-01 |
...
8. policy-backup-auto.bicep (smb-backup-02, depends on backup.bicep outputs)
9. migrate.bicep
...
resource logAnalytics 'Microsoft.OperationalInsights/workspaces@2023-09-01' = {
  name: 'log-smbrf-${environment}-${regionShort}'
  location: location
  properties: {
    sku: {
      name: 'PerGB2018'
...
resource vault 'Microsoft.RecoveryServices/vaults@2024-04-01' = {
  name: 'rsv-smbrf-${environment}-${regionShort}'
  location: location
  sku: {
    name: 'RS0'
    tier: 'Standard'
...
| ---------------- | ------------------------------ | --------------------- | -------------------------------- |
| Resource Group   | rg-{workload}-{env}-{region}   | rg-hub-smb-swc        | Shared services use `smb` as env |
| Virtual Network  | vnet-{workload}-{env}-{region} | vnet-hub-smb-swc      | Hub uses `smb`, spoke uses env   |
| Subnet           | snet-{purpose}                 | snet-workload         |                                  |
| NSG              | nsg-{workload}-{env}-{region}  | nsg-hub-smb-swc       | Shared services use `smb`        |
...
| Azure Bastion    | bas-{workload}-{env}-{region}  | bas-hub-smb-swc       | Shared services use `smb`        |
| Azure Firewall   | fw-{workload}-{env}-{region}   | fw-hub-smb-swc        | Shared services use `smb`        |
| VPN Gateway      | vpng-{workload}-{env}-{region} | vpng-hub-smb-swc      | Shared services use `smb`        |
...
| Storage (future) | TLS Version      | 1.2+                     | ✅ smb-storage-03 |
| Storage (future) | Public Access    | Denied                   | ✅ smb-storage-02 |
