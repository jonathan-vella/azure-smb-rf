"""
Azure Architecture Diagram: SMB Ready Foundations - Baseline Scenario
Generated by Diagram agent
Date: 2026-01-29

Scenario: Baseline (NAT Gateway only)
Monthly Cost: ~$48
Features: NAT Gateway for outbound, no Firewall, no VPN, no VNet peering

Prerequisites:
- pip install diagrams
- Graphviz installed (choco install graphviz / brew install graphviz / apt-get install graphviz)

Generate PNG: python 03-des-diagram-baseline.py
"""
...
with Diagram(
    "SMB Ready Foundations - Baseline Scenario (~$48/mo)",
    show=False,
    direction="TB",
    filename="03-des-diagram-baseline",
    ...
    with Cluster("Management Group: smb-rf (SMB Ready Foundations)"):
        ...
        # Spoke Resource Group - ISOLATED (no peering)
        with Cluster("rg-spoke-prod-swc (isolated)"):

            with Cluster("Spoke VNet (10.0.2.0/23)"):
    ...
    # VMs use NAT Gateway for outbound (direct internet)
