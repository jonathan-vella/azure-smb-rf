...
│   ├── resource-groups.bicep      # 5 resource groups [⚠️ Migrate to AVM 0.4.3]
│   ├── networking-hub.bicep       # Hub VNet, NSG, Bastion, DNS [⚠️ Migrate to AVM]
│   ├── networking-spoke.bicep     # Spoke VNet, NSG, NAT Gateway [⚠️ Migrate to AVM]
│   ├── networking-peering.bicep   # VNet peering hub→spoke [Raw - inline]
│   ├── networking-peering-spoke.bicep # VNet peering spoke→hub [Raw - inline]
│   ├── route-tables.bicep         # UDR for firewall routing [⚠️ Migrate to AVM 0.5.0]
│   ├── monitoring.bicep           # Log Analytics Workspace [⚠️ Migrate to AVM 0.15.0]
│   ├── backup.bicep               # Recovery Services Vault [⚠️ Migrate to AVM 0.11.1]
│   ├── migrate.bicep              # Azure Migrate Project [Raw - no AVM exists]
│   ├── budget.bicep               # Cost Management Budget [⚠️ Migrate to AVM 0.3.8]
│   ├── firewall.bicep             # Azure Firewall Basic [✅ Using AVM 0.9.2]
│   └── vpn-gateway.bicep          # VPN Gateway [⚠️ Migrate to AVM 0.10.1]
├── scripts/
