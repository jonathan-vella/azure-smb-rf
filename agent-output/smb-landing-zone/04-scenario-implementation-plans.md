# Scenario Implementation Plans - SMB Landing Zone
...
Landing Zone. Each scenario is a valid production configuration with different cost/capability
trade-offs.
...
| 2   | VPN Only                    |    ❌    | ✅  |   ❌   |   ✅    | ❌  | ~$187        |
| 3   | Firewall + VPN              |    ✅    | ✅  |   ❌   |   ✅    | ✅  | ~$476        |
| 4   | NAT Gateway Only (Baseline) |    ❌    | ❌  |   ✅   |   ❌    | ❌  | ~$48         |
...
graph TB
    ...
    subgraph HUB["Hub VNet (10.0.0.0/16)"]
        FW["🔥 Azure Firewall Basic<br/>10.0.2.0/26"]
        FWMGMT["Firewall Mgmt<br/>10.0.2.64/26"]
        BAS["🏰 Bastion Developer<br/>10.0.1.0/26"]
        DNS["🔒 Private DNS Zone"]
    end

    subgraph SPOKE["Spoke VNet (10.1.0.0/16)"]
        WORKLOAD["💼 Workload Subnet<br/>10.1.1.0/24"]
        DATA["💾 Data Subnet<br/>10.1.2.0/24"]
    end
...
| 4   | Hub VNet                | virtualNetworks        | N/A       | rg-hub-slz-swc     |
| 5   | Spoke VNet              | virtualNetworks        | N/A       | rg-spoke-prod-swc  |
| 6   | Azure Bastion           | bastionHosts           | Developer | rg-hub-slz-swc     |
| 7   | Private DNS Zone        | privateDnsZones        | N/A       | rg-hub-slz-swc     |
| 8   | **Azure Firewall**      | azureFirewalls         | **Basic** | rg-hub-slz-swc     |
| 9   | Firewall Policy         | firewallPolicies       | Basic     | rg-hub-slz-swc     |
| 10  | Firewall Public IP      | publicIPAddresses      | Standard  | rg-hub-slz-swc     |
| 11  | Firewall Mgmt Public IP | publicIPAddresses      | Standard  | rg-hub-slz-swc     |
| 12  | **Route Tables (UDR)**  | routeTables            | N/A       | rg-hub-slz-swc     |
| 13  | **VNet Peering**        | virtualNetworkPeerings | N/A       | Both VNets         |
| 14  | Log Analytics           | workspaces             | PerGB2018 | rg-monitor-slz-swc |
| 15  | Recovery Vault          | vaults                 | Standard  | rg-backup-slz-swc  |
| 16  | Migrate Project         | migrateProjects        | N/A       | rg-migrate-slz-swc |
...
graph TB
    ...
    subgraph HUB["Hub VNet (10.0.0.0/16)"]
        VPN["🔐 VPN Gateway VpnGw1AZ<br/>10.0.0.128/27"]
        BAS["🏰 Bastion Developer<br/>10.0.1.0/26"]
        DNS["🔒 Private DNS Zone"]
    end
...
| 6   | Azure Bastion           | bastionHosts           | Developer    | rg-hub-slz-swc     |
| 7   | Private DNS Zone        | privateDnsZones        | N/A          | rg-hub-slz-swc     |
| 8   | **VPN Gateway**         | virtualNetworkGateways | **VpnGw1AZ** | rg-hub-slz-swc     |
| 9   | VPN Gateway Public IP   | publicIPAddresses      | Standard     | rg-hub-slz-swc     |
| 10  | **VNet Peering**        | virtualNetworkPeerings | N/A          | Both VNets         |
| 11  | Log Analytics           | workspaces             | PerGB2018    | rg-monitor-slz-swc |
| 12  | Recovery Vault          | vaults                 | Standard     | rg-backup-slz-swc  |
| 13  | Migrate Project         | migrateProjects        | N/A          | rg-migrate-slz-swc |
...
| 8   | **Azure Firewall**      | azureFirewalls         | **Basic**    | rg-hub-slz-swc     |
| 9   | Firewall Policy         | firewallPolicies       | Basic        | rg-hub-slz-swc     |
| 10  | Firewall Public IPs (2) | publicIPAddresses      | Standard     | rg-hub-slz-swc     |
| 11  | **VPN Gateway**         | virtualNetworkGateways | **VpnGw1AZ** | rg-hub-slz-swc     |
| 12  | VPN Gateway Public IP   | publicIPAddresses      | Standard     | rg-hub-slz-swc     |
| 13  | **Route Tables (2)**    | routeTables            | N/A          | rg-hub-slz-swc     |
| 14  | **VNet Peering**        | virtualNetworkPeerings | N/A          | Both VNets         |
| 15  | Log Analytics           | workspaces             | PerGB2018    | rg-monitor-slz-swc |
| 16  | Recovery Vault          | vaults                 | Standard     | rg-backup-slz-swc  |
| 17  | Migrate Project         | migrateProjects        | N/A          | rg-migrate-slz-swc |
...
**Gateway UDR (rt-gateway-slz-swc):**
...
| 10  | Log Analytics           | workspaces        | PerGB2018 | rg-monitor-slz-swc |
| 11  | Recovery Vault          | vaults            | Standard  | rg-backup-slz-swc  |
| 12  | Migrate Project         | migrateProjects   | N/A       | rg-migrate-slz-swc |
...
| ----------------------- | ------------ | ------------------ | ------------------ | ----------- |
| **Egress Filtering**    | ✅ Firewall  | ❌ None            | ✅ Firewall        | ❌ None     |
| **Hybrid Connectivity** | ❌ None      | ✅ VPN             | ✅ VPN             | ❌ None     |
| **Traffic Inspection**  | ✅ All       | ❌ None            | ✅ All             | ❌ None     |
| **Outbound Internet**   | Firewall PIP | Direct             | Firewall PIP       | NAT Gateway |
| **VNet Peering**        | ✅ Yes       | ✅ Gateway Transit | ✅ Gateway Transit | ❌ No       |
| **UDR Required**        | ✅ Yes       | ❌ No              | ✅ Yes             | ❌ No       |
...
| Threat intelligence     | ✅ Alert   | ❌         | ✅ Alert   | ❌         |
...
| --- | ---------------- | ---------- | -------- | --------- |
| 1   | Firewall Only    | 2026-01-29 | 9.3 min  | ✅ Passed |
| 2   | VPN Only         | 2026-01-29 | 23 min   | ✅ Passed |
| 3   | Firewall + VPN   | 2026-01-29 | 9.8 min  | ✅ Passed |
| 4   | NAT Gateway Only | 2026-01-28 | 2.2 min  | ✅ Passed |
...
_Generated by bicep-plan agent | SMB Landing Zone v0.1_
