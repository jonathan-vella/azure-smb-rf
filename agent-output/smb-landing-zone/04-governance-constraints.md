# Governance Constraints - SMB Landing Zone
...
| -------------- | --------------------------- | -------------------------------------- | --------------- |
| Naming         | CAF naming conventions      | Enforced via Bicep variables           | Project-defined |
...
| ----------- | ----------------- | ------ |
| Environment | smb-lz-tagging-01 | Deny   |
| Owner       | smb-lz-tagging-02 | Deny   |
...
| --- | ----------------- | --------------------- | ------ |
| 1   | smb-lz-compute-01 | Allowed VM SKUs       | Deny   |
| 2   | smb-lz-compute-02 | No public IPs on NICs | Deny   |
| 3   | smb-lz-compute-03 | Audit managed disks   | Audit  |
| 4   | smb-lz-compute-04 | Audit ARM VMs         | Audit  |

### Network Guardrails (4 policies)

| #   | Assignment Name   | Policy                 | Effect           |
| --- | ----------------- | ---------------------- | ---------------- |
| 5   | smb-lz-network-01 | NSG on subnets         | AuditIfNotExists |
| 6   | smb-lz-network-02 | Close management ports | AuditIfNotExists |
| 7   | smb-lz-network-03 | Restrict NSG ports     | AuditIfNotExists |
| 8   | smb-lz-network-04 | Disable IP forwarding  | Deny             |

### Storage Guardrails (5 policies)

| #   | Assignment Name   | Policy                  | Effect |
| --- | ----------------- | ----------------------- | ------ |
| 9   | smb-lz-storage-01 | HTTPS only              | Deny   |
| 10  | smb-lz-storage-02 | No public blob access   | Deny   |
| 11  | smb-lz-storage-03 | TLS 1.2 minimum         | Deny   |
| 12  | smb-lz-storage-04 | Restrict network access | Audit  |
| 13  | smb-lz-storage-05 | ARM migration           | Audit  |

### Identity & Access (2 policies)

| #   | Assignment Name    | Policy               | Effect |
| --- | ------------------ | -------------------- | ------ |
| 14  | smb-lz-identity-01 | SQL Azure AD-only    | Audit  |
| 15  | smb-lz-identity-02 | SQL no public access | Audit  |

### Tagging & Governance (3 policies)

| #   | Assignment Name      | Policy                  | Effect |
| --- | -------------------- | ----------------------- | ------ |
| 16  | smb-lz-tagging-01    | Require Environment tag | Deny   |
| 17  | smb-lz-tagging-02    | Require Owner tag       | Deny   |
| 18  | smb-lz-governance-01 | Allowed locations       | Deny   |

### Monitoring & Backup (2 policies)

| #   | Assignment Name      | Policy              | Effect           |
| --- | -------------------- | ------------------- | ---------------- |
| 19  | smb-lz-backup-01     | VM backup required  | AuditIfNotExists |
| 20  | smb-lz-monitoring-01 | Diagnostic settings | AuditIfNotExists |
