...
if ($existingPolicies) {
    ...
    Write-Warning "$policyCount existing smb-* policy assignments found"
...
if ($Scenario -in @('firewall', 'full')) {
    ...
    Remove-FaultedFirewall -ResourceGroupName $hubRg `
        -FirewallName "fw-hub-smb-$regionAbbrev" `
        -PolicyName "fwpol-hub-smb-$regionAbbrev"
}
...
if ($Scenario -in @('vpn', 'full')) {
    ...
    Remove-FaultedVpnGateway -ResourceGroupName $hubRg `
        -GatewayName "vpng-hub-smb-$regionAbbrev" `
        -PublicIpName "pip-vpn-smb-$regionAbbrev"
}
...
if ($Force) {
    ...
    $backupPolicy = az policy assignment show --name 'smb-backup-02' `
        --scope "/subscriptions/$subId" 2>$null
    if ($LASTEXITCODE -eq 0 -and $backupPolicy) {
        az policy assignment delete --name 'smb-backup-02' `
            --scope "/subscriptions/$subId" 2>$null
...
$deploymentName = "smb-$Environment-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
