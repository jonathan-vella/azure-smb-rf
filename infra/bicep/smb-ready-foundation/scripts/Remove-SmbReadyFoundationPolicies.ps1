...
# All policy assignment names (prefixed with 'smb-')
$policyAssignments = @(
    'smb-compute-01'      # Allowed VM SKUs
    'smb-compute-02'      # No public IPs on NICs
    'smb-compute-03'      # Audit managed disks
    'smb-compute-04'      # Audit ARM VMs
    'smb-network-01'      # NSG on subnets
    'smb-network-02'      # Close management ports
    'smb-network-03'      # Restrict NSG ports
    'smb-network-04'      # Disable IP forwarding
    'smb-storage-01'      # HTTPS only
    'smb-storage-02'      # No public blob access
    'smb-storage-03'      # TLS 1.2 minimum
    'smb-storage-04'      # Restrict network access
    'smb-storage-05'      # ARM migration
    'smb-identity-01'     # SQL Azure AD-only
    'smb-identity-02'     # SQL no public access
    'smb-tagging-01'      # Require Environment tag
    'smb-tagging-02'      # Require Owner tag
    'smb-governance-01'   # Allowed locations
    'smb-backup-01'       # VM backup required
    'smb-monitoring-01'   # Diagnostic settings
)
