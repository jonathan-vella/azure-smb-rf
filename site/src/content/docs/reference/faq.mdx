---
title: FAQ
description: Frequently asked questions about SMB Ready Foundation
---
...
A cost-optimized Azure landing zone designed for Microsoft Partners migrating SMB customers from VMware to Azure. It provides hub-spoke networking, 33 governance policies, and 4 deployment scenarios — all deployable with a single `azd up` command.
...
Bastion Developer (free tier) is sufficient for SMB workloads. It provides portal-based SSH/RDP without a dedicated Bastion host. Partners can upgrade to Standard if needed.
