...
description: Common issues and solutions for SMB Ready Foundations deployments
---
...
<Aside type="caution">
  If both `main.bicepparam` and `main.parameters.json` exist, azd uses
  `.bicepparam` and **silently ignores** all `azd env set` values. The
  `.bicepparam` file has been renamed to `.bicepparam.reference` to prevent
  this.
</Aside>
...
### Azure Firewall Deployment Failures
