...
| `owner`                  | `OWNER`                      | string | _(required)_    | Owner email for tags and budget alerts                     |
...
| `budgetAmount`           | _(Bicep default)_            | int    | `500`           | Monthly budget in USD (100–10000)                          |
| `budgetAlertEmail`       | _(= owner)_                  | string | `= owner`       | Budget alert recipient                                     |
| `budgetStartDate`        | _(auto)_                     | string | Current month   | Auto-generated, cannot be customized                       |

<Aside type="caution" title="Bicep defaults vs recommended values">
  The Bicep parameter defaults use `/16` CIDRs for simplicity. For production,
  use `/23` CIDRs to minimize IP waste: - Hub: `10.0.0.0/23` (512 addresses) -
...
<Aside type="note">
  The `budgetAmount` parameter is intentionally not in `main.parameters.json` to
  avoid type mismatch issues (string substitution vs integer). The Bicep default
  of 500 applies unless overridden via `infra.parameters`.
</Aside>
...
- [Management Group & Policies](/azure-smb-rf/deploying/management-group/) — understand the governance policy assignments
- [Troubleshooting](/azure-smb-rf/operating/troubleshooting/) — common deployment issues and fixes
- [Customization](/azure-smb-rf/operating/customization/) — extend with new modules, regions, or policies
