...
description: Management group setup and 34 governance policy assignments
---
...
└── smb-rf (SMB Ready Foundation)
    ├── 34 Azure Policy assignments (30 MG + 4 subscription)
...
2. **Step 5b**: Deploys `modules/policy-assignments-mg.bicep` to the `smb-rf` MG — creates 30 MG-scoped policy assignments (3 additional policies are deployed at subscription scope by `main.bicep`)
...
**Total: 34 policy assignments** (9 Deny + 24 Audit + 1 DeployIfNotExists)

The 30 MG-scoped policies are listed above. The remaining 4 are deployed at subscription scope by `main.bicep`: auto-backup enrollment (DINE), VM backup required (Audit), storage geo-redundancy (Audit), and allowed locations (Deny).
...
Management group resources (the MG itself and its 30 MG-scoped policies) persist across `azd up` re-deployments. They are **not** torn down between scenario changes. This is by design:
